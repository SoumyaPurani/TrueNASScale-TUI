...
logger = logging.getLogger("truenasscale_tui.services")
...
class ServicesScreen(Screen):
    ...
    def _get_selected_service(self) -> str | None:
        ...
        try:
            ...
            if not name or name == "No services found":
                return None
            return name
        except Exception:
            return None
    ...
    async def _control_service(self, service: str, action: str) -> None:
        ...
        if not app.connected:
            self.notify("Not connected to TrueNAS", severity="error")
            return
        logger.info("service control: %s %s", action, service)
        try:
            ...
            logger.error("service control failed: %s", exc)
            hint = ""
            if exc.code == -32602:
            ...
            logger.error("service control unexpected error: %s", exc)
